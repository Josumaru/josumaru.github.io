...
const FrequentlyAskQuestion: NextPage = ({}) => {
  return (
    <div className="flex mt-20 flex-col w-full items-center justify-center">
      ...
      />
      <div className="max-w-7xl mt-20 w-full px-3">
        <Accordion type="single" collapsible className="w-full">
          {questions.map((question, index) => {
            return (
              <AccordionItem value={question.question} key={index}>
                <AccordionTrigger className="text-sm md:text-md">{question.question}</AccordionTrigger>
                <AccordionContent className="text-xs md:text-sm text-muted-foreground">{question.answer}</AccordionContent>
              </AccordionItem>
            );
          })}
        </Accordion>
